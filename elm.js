...
Elm.Main.make = function (_elm) {
   ...
   var subtraction = F2(function (x,y) {    return x - y;});
   var division = F2(function (x,y) {    return x / y;});
   var multiply = F2(function (x,y) {    return x * y;});
   var sum = F2(function (x,y) {    return x + y;});
   var initialModel = {total: 0,lastCalculation: 0,input: "",operation: "",allClear: true};
   var Model = F5(function (a,b,c,d,e) {    return {total: a,lastCalculation: b,input: c,operation: d,allClear: e};});
   var parseFloat = function (string) {    var _p0 = $String.toFloat(string);if (_p0.ctor === "Ok") {    return _p0._0;} else {    return 0;}};
   var update = F2(function (action,model) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "AllClear": return initialModel;
         case "Clear": var total = _U.eq(model.input,"") ? 0 : model.total;
           return _U.update(model,{operation: "",input: "",total: total,allClear: true});
         case "UpdateField": return _U.update(model,{input: A2($Basics._op["++"],model.input,_p1._0),allClear: false});
         case "Percent": var total = _U.eq(model.input,"") ? model.total / 100 : model.total;
           var input = !_U.eq(model.input,"") ? $Basics.toString(parseFloat(model.input) / 100) : "";
           return _U.update(model,{input: input,total: total});
         case "Negate": var input = !_U.eq(model.input,"") ? $Basics.toString($Basics.negate(parseFloat(model.input))) : model.input;
           var total = _U.eq(model.input,"") ? $Basics.negate(model.total) : model.total;
           return _U.update(model,{input: input,total: total});
         case "Decimal": var input = _U.eq(model.input,"") && _U.eq(model.total,0) ? ".0" : !_U.eq(model.input,"") ? A2($Basics._op["++"],
           model.input,
           ".") : model.input;
           var total = _U.eq(model.input,"") && _U.eq(model.total,0) ? 0.0 : model.total;
           return _U.update(model,{input: input,total: total});
         case "Sum": return _U.update(model,{operation: "Sum",total: model.total + parseFloat(model.input),input: ""});
         case "Subtract": var total = !_U.eq(model.input,"") && !_U.eq(model.total,0) ? model.total - parseFloat(model.input) : _U.eq(model.input,
           "") && !_U.eq(model.total,0) ? model.total : parseFloat(model.input);
           return _U.update(model,{operation: "Subtract",total: total,input: ""});
         case "Multiply": var total = !_U.eq(model.input,"") ? parseFloat(model.input) : !_U.eq(model.total,0) && !_U.eq(model.input,
           "") ? model.total * parseFloat(model.input) : model.lastCalculation;
           return _U.update(model,{operation: "Multiply",total: total,input: ""});
         case "Divide": var total = !_U.eq(model.input,"") ? parseFloat(model.input) : !_U.eq(model.total,0) && !_U.eq(model.input,
           "") ? model.total / parseFloat(model.input) : model.lastCalculation;
           return _U.update(model,{operation: "Divide",total: total,input: ""});
         default: var _p2 = model.operation;
           switch (_p2)
           {case "Sum": return _U.update(model,
                {operation: "",total: model.total + parseFloat(model.input),lastCalculation: model.total + parseFloat(model.input),input: ""});
              case "Subtract": var total = model.total - parseFloat(model.input);
                return _U.update(model,{operation: "",total: total,lastCalculation: model.total - parseFloat(model.input),input: ""});
              case "Multiply": return _U.update(model,
                {operation: "",total: model.total * parseFloat(model.input),lastCalculation: model.total * parseFloat(model.input),input: ""});
              case "Divide": return _U.update(model,
                {operation: "",total: model.total / parseFloat(model.input),lastCalculation: model.total / parseFloat(model.input),input: ""});
              default: return model;}}
   });
   var classNames = function (strings) {
      return $Html$Attributes.classList(A2($List.map,function (str) {    return {ctor: "_Tuple2",_0: str,_1: true};},strings));
   };
   var renderOutput = function (model) {
      return A2($Html.div,
      _U.list([classNames(_U.list(["output"]))]),
      _U.list([A2($Html.span,
              _U.list([_U.eq(model.input,"") ? classNames(_U.list(["hidden"])) : classNames(_U.list(["visible"]))]),
              _U.list([$Html.text(model.input)]))
              ,A2($Html.span,
              _U.list([_U.eq(model.input,"") ? classNames(_U.list(["visible"])) : classNames(_U.list(["hidden"]))]),
              _U.list([$Html.text($Basics.toString(model.total))]))]));
   };
   var renderOperator = F3(function (val,address,action) {
      return A2($Html.button,_U.list([classNames(_U.list(["btn","btn__operator"])),A2($Html$Events.onClick,address,action)]),_U.list([$Html.text(val)]));
   });
   var renderAction = F3(function (val,address,action) {
      return A2($Html.button,_U.list([classNames(_U.list(["btn","btn__action"])),A2($Html$Events.onClick,address,action)]),_U.list([$Html.text(val)]));
   });
   var decodeButtonText = A2($Json$Decode.at,_U.list(["target","innerText"]),$Json$Decode.string);
   var getTriggeredValue = F2(function (address,f) {
      return A3($Html$Events.on,"click",decodeButtonText,function (v) {    return A2($Signal.message,address,f(v));});
   });
   var renderNumberButton = F2(function (val,address) {
      return A2($Html.button,_U.list([classNames(_U.list(["btn"])),A2(getTriggeredValue,address,UpdateField)]),_U.list([$Html.text(val)]));
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([classNames(_U.list(["model","hidden"]))]),_U.list([$Html.text($Basics.toString(model))]))
              ,A2($Html.div,
              _U.list([classNames(_U.list(["calculator"]))]),
              _U.list([renderOutput(model)
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([A2($Html.button,
                              _U.list([_U.eq(model.allClear,true) ? classNames(_U.list(["btn btn__action"])) : classNames(_U.list(["hidden"]))
                                      ,A2($Html$Events.onClick,address,AllClear)]),
                              _U.list([$Html.text("AC")]))
                              ,A2($Html.button,
                              _U.list([_U.eq(model.allClear,true) ? classNames(_U.list(["hidden"])) : classNames(_U.list(["btn btn__action"]))
                                      ,A2($Html$Events.onClick,address,Clear)]),
                              _U.list([$Html.text("C")]))
                              ,A3(renderAction,"+/-",address,Negate)
                              ,A3(renderAction,"%",address,Percent)
                              ,A3(renderOperator,"/",address,Divide)]))
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([A2(renderNumberButton,"7",address)
                              ,A2(renderNumberButton,"8",address)
                              ,A2(renderNumberButton,"9",address)
                              ,A3(renderOperator,"*",address,Multiply)]))
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([A2(renderNumberButton,"4",address)
                              ,A2(renderNumberButton,"5",address)
                              ,A2(renderNumberButton,"6",address)
                              ,A3(renderOperator,"-",address,Subtract)]))
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([A2(renderNumberButton,"1",address)
                              ,A2(renderNumberButton,"2",address)
                              ,A2(renderNumberButton,"3",address)
                              ,A3(renderOperator,"+",address,Sum)]))
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([A2($Html.button,
                              _U.list([classNames(_U.list(["btn","btn__zero"])),A2(getTriggeredValue,address,UpdateField)]),
                              _U.list([$Html.text("0")]))
                              ,A2($Html.button,
                              _U.list([classNames(_U.list(["btn","btn_zero"])),A2($Html$Events.onClick,address,Decimal)]),
                              _U.list([$Html.text(".")]))
                              ,A3(renderOperator,"=",address,Equals)]))]))]));
   });
   ...
   return _elm.Main.values = {_op: _op
                             ,onInput: onInput
                             ,decodeButtonText: decodeButtonText
                             ,getTriggeredValue: getTriggeredValue
